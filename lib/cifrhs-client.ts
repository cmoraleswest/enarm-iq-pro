...
export function calcularEstatus(pctScore: number, especialidad: string): {
  ...
  if (!esp) {
    return { estatus: 'NO_SELECCIONADO', label: 'SIN DATOS', color: '#64748b', descripcion: 'Especialidad no encontrada' }
  }

  if (pctScore >= esp.minHistorico) {
    return { estatus: 'SELECCIONADO_1RA', label: 'SELECCIONADO EN 1RA VUELTA', color: '#4ade80', descripcion: `Tu puntaje (${pctScore.toFixed(1)}%) supera el mínimo histórico (${esp.minHistorico}%) para ${esp.nombre}` }
  }

  const umbral2da = esp.minHistorico * 0.85
  if (pctScore >= umbral2da) {
    return { estatus: 'DISPONIBLE_2DA', label: 'DISPONIBLE PARA 2DA VUELTA', color: '#fbbf24', descripcion: `Tu puntaje (${pctScore.toFixed(1)}%) está cerca del umbral (${esp.minHistorico}%). Posibilidad en 2da vuelta.` }
  }

  return { estatus: 'NO_SELECCIONADO', label: 'NO SELECCIONADO', color: '#f87171', descripcion: `Tu puntaje (${pctScore.toFixed(1)}%) está por debajo del umbral mínimo (${esp.minHistorico}%) para ${esp.nombre}` }
}
...
export function calcularPercentil(pctScore: number): number {
  ...
  return Math.max(1, Math.min(99, Math.round(cdfNormal(z) * 100)))
}
